...
class MultiVisit(object):
    ...
    * as an lmfit Parameter object.
    ...
    function of time for each dataset, which is a reasonable approximation for
    most individual CHEOPS visits. 
    ...
    of faint stars in crowded fields, or observations at high ecliptic
    latitude. In this case it may be better to include the best-fit trends
    against roll angle from the last fit stored in the .dataset file in the
    fit to each dataset. This case be done using the keyword argument
    "unwrap=True". This option can be combined with the "unroll=True" option,
    i.e. to use "unroll"  as a small correction to the "unwrap" roll-angle
    decorrelation from the last fit to each data set.
    ...
    For plotting and data output we require the "detrended flux", i.e.
      ...
      flux_d = f_obs - f_fit + f_sys + f_sho 
    ...
    detrended fluxes include transits, eclipses, and any correlated noise
    modelled by f_sho. The detrended fluxes for the best fits to each dataset
    are included in the output lmfit ModelResult object in the attribute
    fluxes_det.

    Return value
    ~~~~~~~~~~~~
    The fitting routines return lmfit MinimizerResult objects with a few extra
    attributes. Samples generated by emcee are returned as a python array in
    the attribute flat_chain instead of a pandas.DataFrame object in the
    attribute flatchain.
...
#
    def __run_emcee__(self, **kwargs):
        ...
        for i,(d,p) in enumerate(zip(self.datasets, plist)):
            ...
            # Now the decorrelation parameters, including arbitary
        ...
        for i, dataset in enumerate(self.datasets):
            ...
            if roll or sho:
                ...
                if return_fit:
                    ...
                    f_det =  lc['flux'] - f_fit + f_sys + f_sho 
